...
%            ft=min([pthr ((eff_porosity(1)-thetaant(1))*dzsoi(1))/dtime ks(1)]);
            ft=min([pthr ks(1)]); 
             ...
             [Am,KK,GG,CC,KKr1,KKr2] = matrices(nl_soil,Cant,kant,dzsoi,dtime,ft,fb,zsoi,krad1,krad2);%Compute matrices                
%             psicom=(Am+CC+KKr)^(-1)*(-GG-KK+CC*psii+KKr*psiroot);
             psicom=(Am+CC+KKr)^(-1)*(GG+KK+CC*psiant-(1/dtime)*(thetaant-thetai)+KKr1*psiroot1+KKr2*psiroot2);                  
...
%             psicom=(Am+CC+KKr)^(-1)*(-GG-KK+CC*psii(2:nl_soil))+KKr*psiroot(2:nl_soil);
             psicom=(Am+CC+KKr)^(-1)*(GG+KK+CC*psiant(1:nl_soil)-(1/dtime)*(thetaant(1:nl_soil)-thetai(1:nl_soil))+...
                 KKr1*psiroot1(1:nl_soil)+KKr2*psiroot2(1:nl_soil));
%             psicom=[HB;psicom];
