function varargout = setup_LAD_profile2(varargin)
...
%
%      SETUP_LAD_PROFILE2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SETUP_LAD_PROFILE2.M with the given input arguments.
%
%      SETUP_LAD_PROFILE2('Property','Value',...) creates a new SETUP_LAD_PROFILE2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before setup_LAD_profile2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to setup_LAD_profile2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help setup_LAD_profile2

% Last Modified by GUIDE v2.5 11-Sep-2013 21:12:49

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @setup_LAD_profile2_OpeningFcn, ...
                   'gui_OutputFcn',  @setup_LAD_profile2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before setup_LAD_profile2 is made visible.
function setup_LAD_profile2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to setup_LAD_profile2 (see VARARGIN)

% Choose default command line output for setup_LAD_profile2
handles.output = hObject;

%load './Temps/temp_variable.mat' 'num_can1' 'num_LAD1' 'dat_LAD2'
load './Temps/temp_variable.mat' 'num_LAD1' 'dat_LAD2'
%num_can2=num_can1;
num_LAD2=num_LAD1;
dat_length = length(dat_LAD2(:,1));

if isstr(num_LAD2) == 1
    LAD_num2 = str2num(num_LAD2);
else
    LAD_num2 = (num_LAD2);
end

if dat_length > LAD_num2
    dat_LAD2 = dat_LAD2(1:LAD_num2,:);
else
	dat_LAD_add = zeros(LAD_num2-dat_length,2);
	dat_LAD2 = [dat_LAD2; dat_LAD_add];
end

set(handles.setup_LAD_tab,'Data',dat_LAD2)
...
% UIWAIT makes setup_LAD_profile2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = setup_LAD_profile2_OutputFcn(hObject, eventdata, handles) 
...
if pathname == 0 %if the user pressed cancelled, then we exit this callback
	...
	load './Temps/temp_variable.mat' 'num_LAD2'
    ...
    if isstr(num_LAD2) == 0
        num_LAD2 = num2str(num_LAD2); 
    end
    
    if length(LAD_data_table(:,1)) ~= str2num(num_LAD2)
        numstr=num2str(length(LAD_data_table(:,1)));
        notice = ['Number of LAD (',num_LAD2,') must be equal to data in imported file (',numstr,')'];
...
dat_LAD2 = get(handles.setup_LAD_tab,'Data');
save './Temps/temp_variable.mat' 'dat_LAD2' -append
close;


function setup_LAD_tab_CellEditCallback(hObject, eventdata, handles)
dat_LAD2        = get(handles.setup_LAD_tab,'Data');
canopy_height   = dat_LAD2(:,1);
LAI             = dat_LAD2(:,2);
