function varargout = setup_root_profile4(varargin)
...
%
%      SETUP_ROOT_PROFILE4('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SETUP_ROOT_PROFILE4.M with the given input arguments.
%
%      SETUP_ROOT_PROFILE4('Property','Value',...) creates a new SETUP_ROOT_PROFILE4 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before setup_root_profile4_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to setup_root_profile4_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help setup_root_profile4

% Last Modified by GUIDE v2.5 19-Jan-2014 02:12:52

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @setup_root_profile4_OpeningFcn, ...
                   'gui_OutputFcn',  @setup_root_profile4_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before setup_root_profile4 is made visible.
function setup_root_profile4_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to setup_root_profile4 (see VARARGIN)

% Choose default command line output for setup_root_profile4
...
% end    
load './Temps/temp_variable.mat' 'opt_root4'
if opt_root4 == 1
	set(handles.pan_root_obs,'Visible','on');
	set(handles.pan_root_eqn,'Visible','off');
	set(handles.but_root_load,'Enable','on');
...
% Load table and information for observed option---------------------------
    load './Temps/temp_variable.mat' 'num_root1' 'dat_root4'
    num_root4=num_root1;
    
    dat_length = length(dat_root4(:,1));
    %root_num4 = str2num(num_root4);

    if isstr(num_root4) == 1
        root_num4 = str2num(num_root4);
    else
        root_num4 = (num_root4);
    end
    
    if dat_length > root_num4
        dat_root4 = dat_root4(1:root_num4,:);
    elseif dat_length < root_num4
        dat_root_add = zeros(root_num4-dat_length,2);
        dat_root4 = [dat_root4; dat_root_add];
    end

    plot(handles.axes_root_plot,dat_root4,dat_root4,'Color', 'r',...
        'LineStyle','--','LineWidth',2,'Marker','s','MarkerEdgeColor','k',...
        'MarkerFaceColor','g','MarkerSize',6);
    
    set(handles.setup_root_tab,'Data',dat_root4)
...
% Load table and information for equation option---------------------------
    load './Temps/temp_variable.mat' 'set_para_root4' 'set_root_func'
    para_root_eqn_name = set_para_root4(:,1);
    setappdata(0,'para_root_eqn_name',para_root_eqn_name);
    set(handles.setup_root_tab_para,'Data',set_para_root4);
...
% Load litter depth
    ...
    set(handles.txt_litterDepth4,'String',litter_depth);   
    set(handles.txt_litterDepth4,'Enable','off');

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes setup_root_profile4 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = setup_root_profile4_OutputFcn(hObject, eventdata, handles) 
...
if pathname == 0 %if the user pressed cancelled, then we exit this callback
    ...
    if length(root_data_table(:,1)) ~= str2num(num_root4)
        numstr=num2str(length(root_data_table(:,1)));
        notice = ['Number of root layer (',num_root4,') must be equal to data in imported file (',numstr,')'];
...
opt_root4       = get(handles.root_rad_obs,'Value');
set_para_root4  = get(handles.setup_root_tab_para,'Data');
set_root_func   = get(handles.setup_root_tab_eqn,'Data');
if opt_root4 == 1
    dat_root4        = get(handles.setup_root_tab,'Data');
else
    zns_eqn         = getappdata(0,'zns');
    rootfr_eqn      = getappdata(0,'rootfr');
    dat_root4        = [zns_eqn, rootfr_eqn];
end
save './Temps/temp_variable.mat'...
        'dat_root4' 'opt_root4' 'set_para_root4' 'set_root_func' 'num_root_eqn' -append
close;


function setup_root_tab_CellEditCallback(hObject, eventdata, handles)
dat_root4    = get(handles.setup_root_tab,'Data');
root_depth  = dat_root4(:,1);
RLD         = dat_root4(:,2);
...
function txt_litterDepth4_Callback(hObject, eventdata, handles)
% hObject    handle to txt_litterDepth4 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of txt_litterDepth4 as text
%        str2double(get(hObject,'String')) returns contents of txt_litterDepth4 as a double


% --- Executes during object creation, after setting all properties.
function txt_litterDepth4_CreateFcn(hObject, eventdata, handles)
% hObject    handle to txt_litterDepth4 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
