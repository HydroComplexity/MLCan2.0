...
load './Temps/temp_variable.mat' 'num_LAD1' 'dat_LAD1'
dat_length = length(dat_LAD1(:,1));

if isstr(num_LAD1) == 1
    LAD_num1 = str2num(num_LAD1);
else
    LAD_num1 = (num_LAD1);
end

if dat_length > LAD_num1
    dat_LAD1 = dat_LAD1(1:LAD_num1,:);
else
	dat_LAD_add = zeros(LAD_num1-dat_length,2);
	dat_LAD1 = [dat_LAD1; dat_LAD_add];
end

set(handles.setup_LAD_tab,'Data',dat_LAD1)
...
if pathname == 0 %if the user pressed cancelled, then we exit this callback
	...
	load './Temps/temp_variable.mat' 'num_LAD1'
    ...
    if length(LAD_data_table(:,1)) ~= str2num(num_LAD1)
        numstr=num2str(length(LAD_data_table(:,1)));
        notice = ['Number of LAD (',num_LAD1,') must be equal to data in imported file (',numstr,')'];
...
dat_LAD1 = get(handles.setup_LAD_tab,'Data');
save './Temps/temp_variable.mat' 'dat_LAD1' -append
close;


function setup_LAD_tab_CellEditCallback(hObject, eventdata, handles)
dat_LAD1        = get(handles.setup_LAD_tab,'Data');
canopy_height   = dat_LAD1(:,1);
LAI             = dat_LAD1(:,2);
