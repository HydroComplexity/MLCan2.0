...
% Last Modified by GUIDE v2.5 19-Jan-2014 02:05:55
...
% end    
load './Temps/temp_variable.mat' 'opt_root1'
if opt_root1 == 1
	set(handles.pan_root_obs,'Visible','on');
	set(handles.pan_root_eqn,'Visible','off');
	set(handles.but_root_load,'Enable','on');
...
% Load table and information for observed option---------------------------
    load './Temps/temp_variable.mat' 'num_root1' 'dat_root1'
    dat_length = length(dat_root1(:,1));
    %root_num1 = str2num(num_root1);
    ...
    if dat_length > root_num1
        dat_root1 = dat_root1(1:root_num1,:);
    elseif dat_length < root_num1
        dat_root_add = zeros(root_num1-dat_length,2);
        dat_root1 = [dat_root1; dat_root_add];
    end

    plot(handles.axes_root_plot,dat_root1,dat_root1,'Color', 'r',...
        'LineStyle','--','LineWidth',2,'Marker','s','MarkerEdgeColor','k',...
        'MarkerFaceColor','g','MarkerSize',6);
    
    set(handles.setup_root_tab,'Data',dat_root1)
...
% Load table and information for equation option---------------------------
    load './Temps/temp_variable.mat' 'set_para_root1' 'set_root_func'
    para_root_eqn_name = set_para_root1(:,1);
    setappdata(0,'para_root_eqn_name',para_root_eqn_name);
    set(handles.setup_root_tab_para,'Data',set_para_root1);
...
if pathname == 0 %if the user pressed cancelled, then we exit this callback
    ...
    if length(root_data_table(:,1)) ~= str2num(num_root1)
...
opt_root1        = get(handles.root_rad_obs,'Value');
set_para_root1   = get(handles.setup_root_tab_para,'Data');
...
if opt_root1 == 1
    ...
    dat_root1        = [zns_eqn, rootfr_eqn];
end
save './Temps/temp_variable.mat'...
        'dat_root1' 'opt_root1' 'set_para_root1' 'set_root_func' 'litter_depth' 'num_root_eqn' -append
close;
