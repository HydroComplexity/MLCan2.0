function varargout = setup_root_profile2(varargin)
...
%
%      SETUP_ROOT_PROFILE2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SETUP_ROOT_PROFILE2.M with the given input arguments.
%
%      SETUP_ROOT_PROFILE2('Property','Value',...) creates a new SETUP_ROOT_PROFILE2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before setup_root_profile2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to setup_root_profile2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help setup_root_profile2

% Last Modified by GUIDE v2.5 19-Jan-2014 02:12:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @setup_root_profile2_OpeningFcn, ...
                   'gui_OutputFcn',  @setup_root_profile2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before setup_root_profile2 is made visible.
function setup_root_profile2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to setup_root_profile2 (see VARARGIN)

% Choose default command line output for setup_root_profile2
...
% end    
load './Temps/temp_variable.mat' 'opt_root2'
if opt_root2 == 1
	set(handles.pan_root_obs,'Visible','on');
	set(handles.pan_root_eqn,'Visible','off');
	set(handles.but_root_load,'Enable','on');
...
%    load './Temps/temp_variable.mat' 'num_root2' 'dat_root2'
    load './Temps/temp_variable.mat' 'num_root1' 'dat_root2'
    num_root2 = num_root1;
    
    dat_length = length(dat_root2(:,1));
    %root_num2 = str2num(num_root2);
    %root_num2 = (num_root2);
    
    if isstr(num_root2) == 1
        root_num2 = str2num(num_root2);
    else
        root_num2 = (num_root2);
    end
    
    if dat_length > root_num2
        dat_root2 = dat_root2(1:root_num2,:);
    elseif dat_length < root_num2
        dat_root_add = zeros(root_num2-dat_length,2);
        dat_root2 = [dat_root2; dat_root_add];
    end

    plot(handles.axes_root_plot,dat_root2,dat_root2,'Color', 'r',...
        'LineStyle','--','LineWidth',2,'Marker','s','MarkerEdgeColor','k',...
        'MarkerFaceColor','g','MarkerSize',6);
    
    set(handles.setup_root_tab,'Data',dat_root2)
...
% Load table and information for equation option---------------------------
    load './Temps/temp_variable.mat' 'set_para_root2' 'set_root_func'
    para_root_eqn_name = set_para_root2(:,1);
    setappdata(0,'para_root_eqn_name',para_root_eqn_name);
    set(handles.setup_root_tab_para,'Data',set_para_root2);
...
% Load litter depth
    ...
    set(handles.txt_litterDepth2,'String',litter_depth);   
    set(handles.txt_litterDepth2,'Enable','off');
    
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes setup_root_profile2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = setup_root_profile2_OutputFcn(hObject, eventdata, handles) 
...
if pathname == 0 %if the user pressed cancelled, then we exit this callback
	...
	load './Temps/temp_variable.mat' 'num_root2'
    ...
    if length(root_data_table(:,1)) ~= str2num(num_root2)
...
opt_root2       = get(handles.root_rad_obs,'Value');
set_para_root2  = get(handles.setup_root_tab_para,'Data');
set_root_func   = get(handles.setup_root_tab_eqn,'Data');
if opt_root2 == 1
    dat_root2        = get(handles.setup_root_tab,'Data');
else
    zns_eqn         = getappdata(0,'zns');
    rootfr_eqn      = getappdata(0,'rootfr');
    dat_root2        = [zns_eqn, rootfr_eqn];
end
save './Temps/temp_variable.mat'...
        'dat_root2' 'opt_root2' 'set_para_root2' 'set_root_func' 'num_root_eqn' -append
close;


function setup_root_tab_CellEditCallback(hObject, eventdata, handles)
dat_root1    = get(handles.setup_root_tab,'Data');
root_depth  = dat_root1(:,1);
RLD         = dat_root1(:,2);
...
function txt_litterDepth2_Callback(hObject, eventdata, handles)
% hObject    handle to txt_litterDepth2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of txt_litterDepth2 as text
%        str2double(get(hObject,'String')) returns contents of txt_litterDepth2 as a double


% --- Executes during object creation, after setting all properties.
function txt_litterDepth2_CreateFcn(hObject, eventdata, handles)
% hObject    handle to txt_litterDepth2 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
